...
- **Pexels API key in `local.properties`:** The API key is loaded at build time from `local.properties` (`PEXELS_API_KEY=...`) or the `PEXELS_API_KEY` environment variable into generated `AppConfig.PEXELS_API_KEY` to keep secrets out of git. Do not commit API keys to source files. If the key is rejected with **401 on uncached requests**, obtain a free key from pexels.com/api and place it in `local.properties`.
