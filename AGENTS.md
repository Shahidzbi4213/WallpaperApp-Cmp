...
- **The committed Pexels API key is dead.** `NetworkModule.kt` carries a shared demo key that Pexels now rejects with **401 on every uncached request**. It looks intermittently alive only because Cloudflare serves stale cached responses for popular URLs — a 200 comes back with `cf-cache-status: HIT` and an `age` of hours or days, while anything genuinely new gets `BYPASS` + 401. Diagnose with `curl -sD- -H "Authorization: <key>" '<url>' | grep -iE 'cf-cache-status|age:'`; if you are debugging "why does page 1 work but page 4 not", this is why. Get a free key from pexels.com/api and replace it.
- **401 is deliberately not retried.** A rejected key is rejected every time, so retrying only delays the error. Retries cover 429 and 5xx. Note that in Ktor, `retryIf` *replaces* the predicate from `retryOnServerErrors` — use one combined predicate or server errors silently stop being retried.
...
- **Desktop uses numbered pages, not infinite scroll.** `ui/desktop/paging/WallpaperPageLoader` fetches one page at a time straight from `PexelWallpapersApi` (40 per page, `DESKTOP_PER_PAGE`) and deliberately bypasses Paging 3 — Paging models an endlessly appending list, and the Pexels endpoints are already page-indexed with `total_results`. Mobile keeps its Paging 3 path. Total pages come from `total_results`, but an empty response clamps the count for the rest of that feed's session, because `total_results` overstates the real depth.
